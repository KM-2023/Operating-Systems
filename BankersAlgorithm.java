...
/* To implement Banker's algorithm the initial values to the allocation, maximum_neededimum need, available matrix were taken from the tutorial on Banker's algorithm.*/
	...
	int n = 1;          // Number of processes: P1 - P5
...
//==========================================================================================================================================================
	...
	// a function to reorder the maximum_need and allocation metrices when invoked 
	public int [][] reorder_process_metrices(int[] desired_order_list, int[] p1,int[] p2, int[] p3,int[] p4, int[] p5) {
		
		int [][] ordered_list = new int[n][m];
		
		// a for loop that takes the order specified by the user and feed it into
				for (int j=0 ; j<desired_order_list.length; j++) {
					int u = desired_order_list[j];
					if (u == 1){
						ordered_list[j] = p1;
					}
					else if (u == 2){
   ...
   //	a method that initializes the values of the allocation, maximum_neededimum need, availability and remaining need metrices. 
	void initializemetrices()
		...
		//int[] available_resources = new int[] {1 , 0,  2};
		          ...
		          /** Multiprocess No-deadlock processes **/
